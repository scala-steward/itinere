...
trait Http4sServerRequest extends HttpRequestAlgebra with Http4sServerUrl { self: Http4sServer =>
  ...
  override def request[A, B, C, AB](method: Method, url: Url[A], headers: HttpRequestHeaders[B], entity: HttpRequestEntity[C])
                                   (implicit T: Tupler.Aux[A, B, AB], TO: Tupler[AB, C]): HttpRequest[TO.Out] = Kleisli { req =>
  ...
  override implicit val httpRequestHeaderValueInvariant: Invariant[Read] = Read.readInvariant
  override implicit val httpRequestHeaderValuePartial: Partial[Read] = Read.readPartial
}
