...
object Json extends JsonDslFormatN { self =>
  ...
  val string: Json[String] = new Json[String] {
    override def apply[F[_] : JsonAlgebra]: F[String] = implicitly[JsonAlgebra[F]].string(StringDescription.Unbounded)
  }
  ...
  val float: Json[Float] = new Json[Float] {
    override def apply[F[_] : JsonAlgebra]: F[Float] = implicitly[JsonAlgebra[F]].float(Range(Bound(BigDecimal(Float.MinValue), true), Bound(BigDecimal(Float.MaxValue), true)))
  }
